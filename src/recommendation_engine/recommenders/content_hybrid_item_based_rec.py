...
class ContentHybridItemBasedRec(AbstractRecommenderDataLoader):
    """
    ...
    def __init__(self):
        ...
        self.sbert_weight = 0.7
        self.use_gpu = torch.cuda.is_available()
        self.model_dir = DATASET_DIR / "models" / "content_hybrid_item_based_rec"
    ...
    def evaluate(self, dataset: str = 'test') -> float:
        """
        Evaluate the model on the pre-processed data.
        """
        pass
    ...
    def get_model(self) -> Any:
        ...
        if os.path.exists(MODEL_DIR):
            ...
            recommender = ContentHybridItemBasedRec.load_model(MODEL_DIR)
        else:
    ...
    def recommend(self,
        ...
        """
        Generate recommendations based on item features using content-based filtering.
        
        Args:
            user_id: Not used in this content-based implementation
            item_features: Dictionary of item features to find similar items for
            user_preferences: Not used in this content-based implementation
            num_recommendations: Number of recommendations to return
            **kwargs: Additional parameters:
                     - query_text: Text query to find relevant items
                     - item_id: Item ID to find similar items for
            
        Returns:
            List of dictionaries containing recommended items with their metadata
        """
        # Handle text query recommendations
        if 'query_text' in kwargs:
            return self.get_recommendations_by_text(
                query_text=kwargs['query_text'],
                n_recommendations=num_recommendations
            )
            
        # Handle similar item recommendations
        if 'item_id' in kwargs:
            return self.get_similar_items(
                item_id=kwargs['item_id'],
                n_recommendations=num_recommendations
            )
            
        # Handle content-based recommendations from item features
        if item_features:
            # Convert item features to text query
            query_text = ' '.join([str(v) for v in item_features.values()])
            return self.get_recommendations_by_text(
                query_text=query_text,
                n_recommendations=num_recommendations
            )
            
        raise ValueError("Either query_text, item_id or item_features must be provided")
    ...
    def update_user_preferences(self,
        ...
        This implementation stores preferences for potential future use in hybrid recommendations.
    ...
    def load_data(self, product_texts: Dict[str, str], product_asins: List[str], item_details: Dict[str, Dict[str, Any]]) -> None:
        """
        ...
        if os.path.exists(PROCESSED_COMBINE_DATA_FILE) and os.path.exists(PROCESSED_METADATA_FILE) and os.path.exists(PROCESSED_REVIEWS_FILE):
    ...
    def get_model(self) -> 'ContentHybridItemBasedRec':
        """
        Get the model.
        """
        model = self.load_model(self.model_dir)
        return model
...
if __name__ == "__main__":
    ...
    # Check if we have already processed data
        ...
        recommender.train(save_model=True)
    else:
        logger.error(f"Pre-processed data files not found in {PROCESSED_DIR} - run AmazonDataProcessor to process the data")
        exit(1)
